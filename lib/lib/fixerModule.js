...
module.exports = {
  fix: (url) => {
    if (url.includes('Joan@7315')) {
      return { status: 401, message: '¡Contraseña no ingresada en la URL!' };
    }

    // Patrón para validar y descomponer la URI
    ...
    const correctedUrl = `mongodb+srv://${username}:${password}@${host}/Admin_database?retryWrites=true&w=majority&appName=Admin_db`;
